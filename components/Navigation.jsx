...
export default function Navigation() {
  ...
  return (
    <>
      {/* Logo - Top Left */}
      <div className="fixed top-4 left-4 sm:top-6 sm:left-6 lg:top-8 lg:left-10 z-50 animate-fade-in">
        <Link 
          href="/" 
      ...
      <div className="fixed top-4 right-4 sm:top-6 sm:right-6 lg:top-8 lg:right-10 z-50 hidden lg:block">
        {isMenuOpen ? (
          <nav className="flex items-center gap-1 bg-white/80 backdrop-blur-xl rounded-full px-1.5 py-1.5 shadow-lg border border-white/50 animate-fade-in">
            {navItems.map((item) => (
              <Link
                key={item.name}
                href={item.href}
          ...
          <button
            ...
            className="w-12 h-12 bg-white/80 backdrop-blur-xl rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-all hover:scale-105 border border-white/50 animate-fade-in"
          >
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          {/* Menu Panel */}
          <div className="absolute top-0 right-0 w-full max-w-sm h-full bg-white/95 backdrop-blur-xl shadow-2xl flex flex-col border-l border-white/20 animate-slide-in-right">
            {/* Header spacing */}
            <div className="h-20" />
